...
float3 Material_GetColour(MaterialType material, float3 thePoint)
{
	return material.Colour;
}

float3 CheckerboardMaterial_GetColour(CheckerboardMaterialType material, float3 thePoint)
{
	return ((floor(thePoint.x) % 2 == 0 && floor(thePoint.z) % 2 == 0)
		|| (abs(floor(thePoint.x)) % 2 == 1 && abs(floor(thePoint.z)) % 2 == 1))
...
float3 PointLight_GetDirectionToLight(PointLight light, float3 thePoint)
{
	return normalize(light.Location - thePoint);
}
